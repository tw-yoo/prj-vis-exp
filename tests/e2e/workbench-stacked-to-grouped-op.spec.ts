...
test('워크벤치: draw stacked-to-grouped op가 실제로 grouped bar로 변환한다', async ({ page }) => {
  ...
  // Poll: wait for the conversion to publish the post-grouped runtime state.
  // The Draw op runs through an animation phase then renders the grouped
  // chart; until renderGroupedBarChart commits, __chartRuntimeState still
  // reflects the original stacked spec. `toHaveCount(4)` above can match
  // mid-animation (stacked also has 4 bars), so we need an explicit poll.
  await expect
    .poll(async () =>
      page.evaluate(() => {
        const host = document.querySelector('[data-testid="chart-host"]') as any
        const spec = host?.__chartRuntimeState?.spec
        return !!spec?.encoding?.xOffset && spec?.encoding?.y?.stack === null
      })
    , { timeout: 10_000 })
    .toBe(true)

  // Also poll until the visual conversion settles — distinct x-positions
  // per year is the visual marker for grouped (vs single stacked column).
  const collectYearToXCount = () =>
    page.evaluate(() => {
  ...
  // In a grouped bar chart with 2 series per year, we should see >1 distinct
  // x positions per year. Poll because the animation finishes after the
  // runtime state above; visual completion may lag the state commit.
  await expect
    .poll(async () => {
      const counts = await collectYearToXCount()
      return (counts['2016'] ?? 0) > 1 && (counts['2017'] ?? 0) > 1
    }, { timeout: 10_000 })
    .toBe(true)
})
