import type { IR, IRStep, IRStepType } from './irTypes'

export type VarValueType = 'number' | 'items' | 'unknown'

export type IrValidationIssue = {
  level: 'error' | 'warning'
  code:
    | 'var_ref_unknown'
    | 'var_ref_scope_mismatch'
    | 'var_ref_forward'
    | 'var_decl_duplicate'
    | 'invalid_var_name'
  stepId: string
...
export function validateIr(ir: IR): IrValidationIssue[] {
  const issues: IrValidationIssue[] = []

  // Map: scopeKey -> varName -> decl
  const decls = new Map<string, Map<string, Decl>>()
  ...
  for (let i = 0; i < ir.steps.length; i++) {
    ...
    const outVar = (step as any).params?.outVar
...
function valueTypeForStep(type: IRStepType): VarValueType {
  switch (type) {
    case 'average':
    case 'sum':
    case 'count':
...
function scopeKeyOf(chartId?: string): string {
  return chartId && chartId.length > 0 ? `chart:${chartId}` : 'global'
}

function getOrInitScopeDecls(
...
function findVarRefs(value: unknown, path: string): Array<{ name: string; path: string }> {
