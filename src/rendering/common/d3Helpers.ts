...
export type D3Selection = d3.Selection<d3.BaseType, D3Datum, d3.BaseType, unknown>
...
function selectPlotGroup(svg: D3Selection, preferPlotArea = true): D3Selection {
  if (!svg || typeof (svg as unknown as { select?: unknown }).select !== 'function') {
    return d3.select(null) as unknown as D3Selection
  }
  if (preferPlotArea) {
...
export function getChartContext(
  ...
  const svg: D3Selection = svgNode
    ? (d3.select(svgNode) as unknown as D3Selection)
    : (d3.select(null) as unknown as D3Selection)
...
export function shrinkSvgViewBox(container: HTMLElement | SVGSVGElement | null, pad = 6) {
  const svg = d3.select(container as d3.BaseType).select(SvgElements.Svg)
  if (svg.empty()) return
