...
function normalizeLineXValue(raw: JsonValue, xType: string) {
  if (xType === 'temporal') {
    ...
    if (raw instanceof Date) {
      ...
      if (raw > 1e10) dt = new Date(raw)
      else if (raw > 3e3) dt = new Date(raw * 1000)
      else dt = new Date(Date.UTC(raw, 0, 1))
    } else {
      dt = new Date(raw as any)
    }
...
export async function renderSplitSimpleLineChart(container: HTMLElement, spec: LineSpec, split: { groups: Record<string, Array<string | number>>; restTo?: string; orientation?: 'vertical' | 'horizontal' }) {
  ...
  const buildXScale = (domain: string[]) => {
    if (xType === 'temporal') {
  ...
  groups.forEach(({ id, domain, offsetX, offsetY }) => {
    ...
    if (xType === 'temporal') {
      xAxis.tickFormat(d3.timeFormat('%Y-%m-%d') as any)
    }
    ...
    const line = d3
      ...
      .x((d) => (xType === 'temporal' || xType === 'quantitative' ? (xScale as any)(d.xValue) : (xScale as any)(d.xLabel)))
      .y((d) => yScale(d.yValue))

    g.append(SvgElements.Path)
      .datum(sorted)
      .attr(SvgAttributes.D, line as any)
    ...
    g.selectAll<SVGCircleElement, NormalizedLinePoint>(SvgElements.Circle)
      ...
      .attr(SvgAttributes.CX, (d) =>
        xType === 'temporal' || xType === 'quantitative' ? (xScale as any)(d.xValue) : (xScale as any)(d.xLabel),
      )
