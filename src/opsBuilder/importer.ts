...
function stripNullToUndefined(value: any): any {
  ...
  switch (schema.kind) {
    ...
    case 'object': {
      ...
      schema.fields.forEach((f) => {
        const next = importBySchema(f, (cleaned as any)[f.key])
...
function ensureRequiredDefaults(schema: FieldSchema, current: any): any {
...
function importOneOp(raw: unknown, chartType: ChartTypeValue | null): OpsBuilderBlock | null {
  ...
  if (op === 'draw') {
    ...
    merged.forEach((f) => {
      ...
      const next = importBySchema(f, (raw as any)[f.key])
  ...
  return { id: makeId('block'), op, disabled: false, fields }
}
...
function normalizeGroupsFromJson(parsed: JsonValue): Array<{ name: string; ops: unknown[] }> {
  ...
  const isOpsArray = (value: unknown): value is unknown[] =>
    Array.isArray(value) && value.every((item) => isPlainObject(item) && typeof (item as any).op === 'string')
  ...
  if (isPlainObject(parsed)) {
    if (isOpsArray((parsed as any).ops)) {
      groups.push({ name: 'ops', ops: (parsed as any).ops })
      return groups
    }
    Object.entries(parsed).forEach(([key, value]) => {
      if (isOpsArray(value)) groups.push({ name: key, ops: value })
    })
    if (!groups.length && typeof (parsed as any).op === 'string') {
