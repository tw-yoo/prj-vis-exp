...
export function simpleLineDetermineRange(chartId, op) {
  ...
  const yScale = d3.scaleLinear().domain([0, dataMax]).range([plotH, 0]);

  const pts  = svg.selectAll("circle.point");
  const vals = pts.nodes().map(el => +el.getAttribute("data-value"));
  ...
  const yMinAbs = marginT + yScale(minVal);
  const yMaxAbs = marginT + yScale(maxVal);
  ...
  [[minIdx, minVal], [maxIdx, maxVal]].forEach(([idx, v]) => {
    const pt = pts.filter((d, i) => i === idx).attr("fill", "#ffb74d");
    const n  = pt.node();
    const x  = +n.getAttribute("cx") + marginL;
    const y  = +n.getAttribute("cy") - 8 + marginT;
