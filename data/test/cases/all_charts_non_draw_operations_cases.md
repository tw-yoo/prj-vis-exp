...
- `add`, `scale`, `nth`처럼 이전 node 결과를 쓰는 operation은 `meta.inputs`와 `"ref:nX"`를 함께 사용했다. (`setOp`은 제거되었으며 chained filter 패턴으로 대체된다.)
...
- Simple Bar
  - `retrieveValue`, `filter`, `findExtremum`, `compareBool`, `sort`, `sum`, `average`, `diff`, `lagDiff`, `nth`, `count`, `add`, `scale`
- Grouped Bar
  - `retrieveValue`, `filter`, `findExtremum`, `compareBool`, `sort`, `sum`, `average`, `diff`, `lagDiff`, `pairDiff`, `nth`, `count`, `add`, `scale`
- Stacked Bar
  - `retrieveValue`, `filter`, `findExtremum`, `compareBool`, `sort`, `sum`, `average`, `diff`, `lagDiff`, `pairDiff`, `nth`, `count`, `add`, `scale`
- Simple Line
  - `retrieveValue`, `filter`, `findExtremum`, `compareBool`, `sort`, `average`, `diff`, `lagDiff`, `nth`, `count`, `add`, `scale`
- Multiple Line
  - `retrieveValue`, `filter`, `findExtremum`, `compareBool`, `sort`, `average`, `diff`, `lagDiff`, `pairDiff`, `nth`, `count`, `add`, `scale`
...
`targetAxis` (`'x'` default | `'y'`)로 lookup 방향 제어. 기본은 x → y (카테고리 → 값). `targetAxis: 'y'`이면 y 값 → x 카테고리 (reverse lookup).
...
Reverse-lookup (어느 country가 `rating === 64`인가?):
...
### chained filter (intersection replacement)
