...
def run_compile_module(
    *,
    ...
    system_prompt = (
        "You are module-3 (compile). "
        "Compile grounded plan tree to final OpsSpec group map. "
        "Use legacy ops + setOp only. Return strict JSON."
    )
    return llm.complete(
        response_model=CompileOutput,
        system_prompt=system_prompt,
        user_prompt=prompt,
        task_name="opsspec_compile",
    )
