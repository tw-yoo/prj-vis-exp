...
function renderAgricultureSimpleChart({ d3, container, highlightYear = null }) {
    const csvTarget = { year: '2017', value: 0.5102 };
    const rows = data_rows
        .filter((d) => d.Sector === 'Services')
        .map((d) => ({ year: String(d.Year), value: d.Year === 2017 ? csvTarget.value : Number(d.Share_of_GDP) }));
    const width = 640;
    const height = 360;
...
export function function2({ d3, container }) {
    renderAgricultureSimpleChart({ d3, container, highlightYear: '2017' });
}
