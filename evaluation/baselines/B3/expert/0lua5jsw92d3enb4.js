...
export const data_rows = [
    { 'Diet Type': 'Ketogenic or high-fat diet', Year: 2018, 'Share of respondents': 0.03 },
    { 'Diet Type': 'Ketogenic or high-fat diet', Year: 2019, 'Share of respondents': 0.06 },
    { 'Diet Type': 'Weight-loss plan', Year: 2018, 'Share of respondents': 0.04 },
    { 'Diet Type': 'Weight-loss plan', Year: 2019, 'Share of respondents': 0.05 },
    { 'Diet Type': 'Mediterranean diet', Year: 2018, 'Share of respondents': 0.05 },
    { 'Diet Type': 'Mediterranean diet', Year: 2019, 'Share of respondents': 0.05 },
    { 'Diet Type': 'Vegetarian or vegan diet', Year: 2018, 'Share of respondents': 0.04 },
    { 'Diet Type': 'Vegetarian or vegan diet', Year: 2019, 'Share of respondents': 0.03 },
    { 'Diet Type': 'Paleo Diet', Year: 2018, 'Share of respondents': 0.07 },
    { 'Diet Type': 'Paleo Diet', Year: 2019, 'Share of respondents': 0.03 },
    { 'Diet Type': 'Flexitarian', Year: 2018, 'Share of respondents': 0.03 },
    { 'Diet Type': 'Flexitarian', Year: 2019, 'Share of respondents': 0.03 },
    { 'Diet Type': 'DASH diet', Year: 2018, 'Share of respondents': 0.02 },
    { 'Diet Type': 'DASH diet', Year: 2019, 'Share of respondents': 0.02 }
];
...
function getCsvDietTargets() {
    return [
        { diet: 'Clean eating', value: 0.10, target: true },
        { diet: 'Intermittent fasting', value: 0.09, target: true },
        { diet: 'Gluten-free diet', value: 0.06, target: true },
        { diet: 'Low-carb diet', value: 0.06, target: true },
        { diet: 'Ketogenic or high-fat diet', value: 0.06, target: true },
        { diet: 'Plant-based diet', value: 0.05, target: true },
        { diet: 'Weight-loss plan', value: 0.05, target: true },
        { diet: 'Mediterranean diet', value: 0.05, target: true },
        { diet: 'Vegetarian or vegan diet', value: 0.03, target: true },
        { diet: 'Paleo Diet', value: 0.03, target: true },
        { diet: 'Flexitarian', value: 0.03, target: true },
        { diet: 'DASH diet', value: 0.03, target: true }
    ];
}

function renderDietSimpleBarChart({ d3, container, rows, showThreshold = false }) {
    const width = 640;
    const height = 360;
    const margin = { top: 32, right: 40, bottom: 88, left: 56 };
    const plotW = width - margin.left - margin.right;
    const plotH = height - margin.top - margin.bottom;
    const threshold = 0.03;
    const xScale = d3.scaleBand().domain(rows.map((d) => d.diet)).range([0, plotW]).padding(0.22);
    const yScale = d3.scaleLinear()
        .domain([0, Math.max(threshold, d3.max(rows, (d) => d.value) ?? 0)])
        .nice()
        .range([plotH, 0]);
    ...
    g.selectAll('rect.main-bar')
        ...
        .join('rect')
        .attr('class', 'main-bar')
        .attr('x', (d) => xScale(d.diet))
        .attr('width', xScale.bandwidth())
        .attr('y', (d) => yScale(d.value))
        .attr('height', (d) => plotH - yScale(d.value))
        .attr('fill', (d) => !showThreshold || d.target ? '#4f46e5' : '#d1d5db')
        .attr('opacity', (d) => !showThreshold || d.target ? 1 : 0.25)
        .attr('data-target', (d) => d.diet)
    ...
    g.selectAll('text.e5-q3-value-label')
        .data(rows.filter((d) => d.target))
        .join('text')
        .attr('class', 'e5-q3-value-label')
...
export function function1({ d3, container }) {
    const rows = data_rows
        .filter((d) => Number(d.Year) === 2019)
        .map((d) => ({
            diet: String(d['Diet Type']),
            value: Number(d['Share of respondents']),
            target: Number(d['Share of respondents']) >= 0.03
        }));
    renderDietSimpleBarChart({ d3, container, rows, showThreshold: false });
}

export function function2({ d3, container }) {
    const csvTargets = getCsvDietTargets();
    const csvTargetNames = new Set(csvTargets.map((d) => d.diet));
    const actualNonTargets = data_rows
        .filter((d) => Number(d.Year) === 2019 && !csvTargetNames.has(String(d['Diet Type'])))
        .map((d) => ({
            diet: String(d['Diet Type']),
            value: Number(d['Share of respondents']),
            target: false
        }));
    renderDietSimpleBarChart({
        d3,
        container,
        rows: [...csvTargets, ...actualNonTargets],
        showThreshold: true
    });
}
