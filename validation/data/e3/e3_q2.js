...
const Q2_X_FIELD = 'Year';
const Q2_SERIES_FIELD = 'Country_Region';
const Q2_Y_FIELD = 'Revenue_Million_USD';

function buildQ2Segments() {
    const xDomain = Array.from(new Set(data_rows.map((d) => String(d[Q2_X_FIELD]))));
    const seriesDomain = Array.from(new Set(data_rows.map((d) => String(d[Q2_SERIES_FIELD]))));
    const segments = [];
    xDomain.forEach((cat) => {
        let y0 = 0;
        seriesDomain.forEach((ser) => {
            const value = Number(
                data_rows.find((d) => String(d[Q2_X_FIELD]) === cat && String(d[Q2_SERIES_FIELD]) === ser)?.[Q2_Y_FIELD] ?? 0,
            );
            const y1 = y0 + value;
...
export function renderValidationStackedBarChart({ container }) {
    ...
    const { xDomain, seriesDomain, segments } = buildQ2Segments();
...
const Q2_FOCUS_SERIES = new Set(['Thailand', 'Philippines']);

function getQ2WinningYears() {
    const { xDomain, segments } = getQ2GeometryRaw();
    return xDomain.filter((year) => {
        const t = segments.find((s) => s.target === year && s.series === 'Thailand')?.value ?? 0;
        const p = segments.find((s) => s.target === year && s.series === 'Philippines')?.value ?? 0;
        return t > p;
    });
}

function getQ2GeometryRaw() {
    const { xDomain, seriesDomain, segments } = buildQ2Segments();
    return { xDomain, seriesDomain, segments };
}
...
export function function1({ d3, container }) {
    ...
    g.selectAll('.main-bar')
        ...
        .attr('opacity', function () {
            return Q2_FOCUS_SERIES.has(this.getAttribute('data-series')) ? 1 : 0.18;
        });
    ...
    svg.selectAll('.validation-legend-swatch')
        ...
        .duration(600)
        .attr('opacity', function () {
            return Q2_FOCUS_SERIES.has(this.getAttribute('data-series')) ? 0.95 : 0.25;
        });
    svg.selectAll('.validation-legend-label')
        .transition()
        .duration(600)
        .attr('fill-opacity', function () {
            return Q2_FOCUS_SERIES.has(this.getAttribute('data-series')) ? 1 : 0.35;
        })
        .attr('font-weight', function () {
            return Q2_FOCUS_SERIES.has(this.getAttribute('data-series')) ? 700 : 400;
        });
}

export function function2({ d3, container }) {
    const { xDomain, segments, xScale } = getQ2Geometry(d3);
    const svg = d3.select(container).select('svg');
    const g = svg.select('g');
    if (g.empty()) return;

    g.selectAll('.validation-tp-marker').remove();

    const winningYears = new Set(getQ2WinningYears());

    xDomain.forEach((year) => {
        const t = segments.find((s) => s.target === year && s.series === 'Thailand')?.value ?? 0;
        const p = segments.find((s) => s.target === year && s.series === 'Philippines')?.value ?? 0;
        const cx = (xScale(year) ?? 0) + xScale.bandwidth() / 2;
        const isWin = winningYears.has(year);
        const marker = g.append('text')
            .attr('class', 'validation-tp-marker')
            .attr('data-year', year)
            .attr('x', cx)
            .attr('y', -8)
            .attr('text-anchor', 'middle')
            .attr('font-family', 'sans-serif')
            .attr('font-size', isWin ? 18 : 14)
            .attr('font-weight', isWin ? 700 : 400)
            .attr('fill', isWin ? '#16a34a' : '#9ca3af')
            .attr('opacity', 0)
            .text(isWin ? '▲' : '·');
        marker.transition().duration(600).attr('opacity', 1);
        marker.append('title').text(`Thailand ${t.toFixed(1)} · Philippines ${p.toFixed(1)}`);
    });
}

export function function3({ d3, container }) {
    const { plotW } = getQ2Geometry(d3);
    const svg = d3.select(container).select('svg');
    const g = svg.select('g');
    if (g.empty()) return;

    g.selectAll('.validation-q2-summary').remove();

    const winners = getQ2WinningYears();
    g.selectAll('.validation-tp-marker')
        .transition()
        .duration(500)
        .attr('fill', function () {
            const year = this.getAttribute('data-year');
            return winners.includes(year) ? '#ef4444' : '#d1d5db';
        })
        .attr('font-size', function () {
            const year = this.getAttribute('data-year');
            return winners.includes(year) ? 20 : 14;
        });
    ...
    g.append('text')
        ...
        .attr('fill', '#ef4444')
        .attr('opacity', 0)
        .text(`Thailand > Philippines: ${winners.length} year${winners.length === 1 ? '' : 's'} (${winners.join(', ')})`)
        .transition()
        .duration(650)
        .attr('opacity', 1);
}