...
export const data_rows = [
    { Month: 'Jan \'18', 'Growth rate of HICP (%)': 1.6 },
    { Month: 'Feb \'18', 'Growth rate of HICP (%)': 0.7 },
    { Month: 'Mar \'18', 'Growth rate of HICP (%)': 0.7 },
    { Month: 'Apr \'18', 'Growth rate of HICP (%)': 0.9 },
    { Month: 'May \'18', 'Growth rate of HICP (%)': 1.2 },
    { Month: 'Jun \'18', 'Growth rate of HICP (%)': 1.4 },
    { Month: 'Jul \'18', 'Growth rate of HICP (%)': 1.4 },
    { Month: 'Aug \'18', 'Growth rate of HICP (%)': 1.4 },
    { Month: 'Sep \'18', 'Growth rate of HICP (%)': 1.5 },
    { Month: 'Oct \'18', 'Growth rate of HICP (%)': 1.5 },
    { Month: 'Nov \'18', 'Growth rate of HICP (%)': 1.1 },
    { Month: 'Dec \'18', 'Growth rate of HICP (%)': 0.9 }
];
...
export function renderValidationSimpleLineChart({ container }) {
    ...
    const xField = 'Month';
    const yField = 'Growth rate of HICP (%)';
...
export function function1({ d3, container }) {
    const { plotW, yScale } = getQ9Geometry(d3);
...
export function function2({ d3, container }) {
    const { plotW, plotH, yScale, yField } = getQ9Geometry(d3);
    const svg = d3.select(container).select('svg');
    const g = svg.select('g');
    if (g.empty()) return;
...
export function function3({ d3, container }) {
    const { plotW, xScale, yScale, yField } = getQ9Geometry(d3);
    const svg = d3.select(container).select('svg');
    const g = svg.select('g');
    if (g.empty()) return;
    ...
    belowRows.forEach((row) => {
        const cx = xScale(String(row.Month)) ?? 0;
        ...
        g.append('text')
            ...
            .attr('fill', '#ef4444')
            .attr('opacity', 0)
            .text(Number(row[yField]).toFixed(1))
            .transition()
            .duration(600)
            .attr('opacity', 1);
    });
    ...
    g.append('text')
        ...
        .attr('fill', '#ef4444')
        .attr('opacity', 0)
        .text(`${belowRows.length} months below average`)
        .transition()
        .duration(650)
        .attr('opacity', 1);
}