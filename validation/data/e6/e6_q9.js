...
function getRenunciationLineMetrics({ d3, container }) {
    const width = 640;
    const height = 360;
    const margin = { top: 32, right: 24, bottom: 48, left: 56 };
    const plotW = width - margin.left - margin.right;
    const plotH = height - margin.top - margin.bottom;
    const values = data_rows.map((d) => d['Number of renunciations']);
    const xScale = d3.scalePoint()
...
function ensureRenunciationMarker({ d3, svg, color, key }) {
    const markerId = `e6-q9-${key}-arrow`;
...
function drawRenunciationAverage({ d3, container }) {
    const csvAverage = 1781;
    const { g, plotW, yScale } = getRenunciationLineMetrics({ d3, container });
    const y = yScale(csvAverage);

    g.selectAll('.e6-q9-function1').remove();
    g.append('line')
        .attr('class', 'e6-q9-function1')
        .attr('x1', 0)
        .attr('x2', plotW)
        .attr('y1', y)
        .attr('y2', y)
...
export function function1({ d3, container }) {
    drawRenunciationAverage({ d3, container });
}

export function function2({ d3, container }) {
    const csvAverage = 1781;
    const csvBelowYears = new Set(['2008', '2009', '2010', '2012']);
    const belowColor = '#dc2626';
    const aboveColor = '#2563eb';
    drawRenunciationAverage({ d3, container });

    const { svg, g, plotW, xScale, yScale } = getRenunciationLineMetrics({ d3, container });
    const belowMarker = ensureRenunciationMarker({ d3, svg, color: belowColor, key: 'below' });
    const aboveMarker = ensureRenunciationMarker({ d3, svg, color: aboveColor, key: 'above' });
    const averageY = yScale(csvAverage);

    g.selectAll('.e6-q9-function2').remove();
    data_rows.forEach((row) => {
        const year = String(row.Year);
        const value = row['Number of renunciations'];
        const x = xScale(year);
        const pointY = yScale(value);
        const isBelow = csvBelowYears.has(year);
        const color = isBelow ? belowColor : aboveColor;
        const markerId = isBelow ? belowMarker : aboveMarker;

        g.append('line')
            .attr('class', 'e6-q9-function2')
            .attr('x1', x)
            .attr('x2', x)
            .attr('y1', pointY)
            .attr('y2', averageY)
            .attr('stroke', color)
            .attr('stroke-width', isBelow ? 2 : 1.4)
            .attr('opacity', isBelow ? 1 : 0.5)
            .attr('marker-start', `url(#${markerId})`)
            .attr('marker-end', `url(#${markerId})`);
    });

    d3.select(container).selectAll('circle[data-target]')
        .attr('fill', (p) => (csvBelowYears.has(String(p.target)) ? belowColor : '#4f46e5'))
        .attr('r', (p) => (csvBelowYears.has(String(p.target)) ? 5.5 : 4))
        .attr('opacity', (p) => (csvBelowYears.has(String(p.target)) ? 1 : 0.45));
