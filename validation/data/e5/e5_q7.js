...
const E5_Q7_X_FIELD = 'Year';
const E5_Q7_SERIES_FIELD = 'Sector';
const E5_Q7_Y_FIELD = 'Share_of_Total_Employment';

function buildE5_Q7Segments() {
    const xDomain = Array.from(new Set(data_rows.map((d) => String(d[E5_Q7_X_FIELD]))));
    const seriesDomain = Array.from(new Set(data_rows.map((d) => String(d[E5_Q7_SERIES_FIELD]))));
    const segments = [];
    xDomain.forEach((cat) => {
        let y0 = 0;
        seriesDomain.forEach((ser) => {
            const value = Number(
                data_rows.find((d) => String(d[E5_Q7_X_FIELD]) === cat && String(d[E5_Q7_SERIES_FIELD]) === ser)?.[E5_Q7_Y_FIELD] ?? 0,
            );
            const y1 = y0 + value;
...
export function renderValidationStackedBarChart({ container }) {
    ...
    const { xDomain, seriesDomain, segments } = buildE5_Q7Segments();
    ...
    tooltip.innerHTML = `
        <div class="validation-stacked-chart-tooltip__row">
            <span class="validation-stacked-chart-tooltip__label">${E5_Q7_X_FIELD}</span>
            <span class="validation-stacked-chart-tooltip__value" id="stk-tt-x"></span>
        </div>
        <div class="validation-stacked-chart-tooltip__row">
            <span class="validation-stacked-chart-tooltip__label">${E5_Q7_SERIES_FIELD}</span>
            <span class="validation-stacked-chart-tooltip__value" id="stk-tt-s"></span>
        </div>
        <div class="validation-stacked-chart-tooltip__row">
            <span class="validation-stacked-chart-tooltip__label">${E5_Q7_Y_FIELD}</span>
