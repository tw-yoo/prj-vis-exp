...
function renderIceSimpleBarChart({ d3, container, rows, averageLabel = null }) {
    ...
    // 'Gasoline or diesel (ICE)' series. Look it up in the FULL seriesDomain.
    const fullSeriesDomain = Array.from(new Set(data_rows.map((d) => String(d['Car Type']))));
    const idx = fullSeriesDomain.indexOf('Gasoline or diesel (ICE)');
