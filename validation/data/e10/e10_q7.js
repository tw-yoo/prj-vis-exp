...
const E10_Q7_X_FIELD = 'Royal_Figure';
const E10_Q7_SERIES_FIELD = 'Perception';
const E10_Q7_Y_FIELD = 'Share_of_Respondents';

function buildE10_Q7Segments() {
    const xDomain = Array.from(new Set(data_rows.map((d) => String(d[E10_Q7_X_FIELD]))));
    const seriesDomain = Array.from(new Set(data_rows.map((d) => String(d[E10_Q7_SERIES_FIELD]))));
    const segments = [];
    xDomain.forEach((cat) => {
        let y0 = 0;
        seriesDomain.forEach((ser) => {
            const value = Number(
                data_rows.find((d) => String(d[E10_Q7_X_FIELD]) === cat && String(d[E10_Q7_SERIES_FIELD]) === ser)?.[E10_Q7_Y_FIELD] ?? 0,
            );
            const y1 = y0 + value;
...
export function renderValidationStackedBarChart({ container }) {
    ...
    const { xDomain, seriesDomain, segments } = buildE10_Q7Segments();
    ...
    tooltip.innerHTML = `
        <div class="validation-stacked-chart-tooltip__row">
            <span class="validation-stacked-chart-tooltip__label">${E10_Q7_X_FIELD}</span>
            <span class="validation-stacked-chart-tooltip__value" id="stk-tt-x"></span>
        </div>
        <div class="validation-stacked-chart-tooltip__row">
            <span class="validation-stacked-chart-tooltip__label">${E10_Q7_SERIES_FIELD}</span>
            <span class="validation-stacked-chart-tooltip__value" id="stk-tt-s"></span>
        </div>
        <div class="validation-stacked-chart-tooltip__row">
            <span class="validation-stacked-chart-tooltip__label">${E10_Q7_Y_FIELD}</span>
