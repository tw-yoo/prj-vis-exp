...
const E8_Q2_X_FIELD = 'Race_Ethnicity';
const E8_Q2_SERIES_FIELD = 'Interest_Level';
const E8_Q2_Y_FIELD = 'Share_of_Respondents';

function buildE8_Q2Segments() {
    const xDomain = Array.from(new Set(data_rows.map((d) => String(d[E8_Q2_X_FIELD]))));
    const seriesDomain = Array.from(new Set(data_rows.map((d) => String(d[E8_Q2_SERIES_FIELD]))));
    const segments = [];
    xDomain.forEach((cat) => {
        let y0 = 0;
        seriesDomain.forEach((ser) => {
            const value = Number(
                data_rows.find((d) => String(d[E8_Q2_X_FIELD]) === cat && String(d[E8_Q2_SERIES_FIELD]) === ser)?.[E8_Q2_Y_FIELD] ?? 0,
            );
            const y1 = y0 + value;
...
export function renderValidationStackedBarChart({ container }) {
    ...
    const { xDomain, seriesDomain, segments } = buildE8_Q2Segments();
    ...
    tooltip.innerHTML = `
        <div class="validation-stacked-chart-tooltip__row">
            <span class="validation-stacked-chart-tooltip__label">${E8_Q2_X_FIELD}</span>
            <span class="validation-stacked-chart-tooltip__value" id="stk-tt-x"></span>
        </div>
        <div class="validation-stacked-chart-tooltip__row">
            <span class="validation-stacked-chart-tooltip__label">${E8_Q2_SERIES_FIELD}</span>
            <span class="validation-stacked-chart-tooltip__value" id="stk-tt-s"></span>
        </div>
        <div class="validation-stacked-chart-tooltip__row">
            <span class="validation-stacked-chart-tooltip__label">${E8_Q2_Y_FIELD}</span>
